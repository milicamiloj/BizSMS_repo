...
namespace BizSMS.Controllers
{
    public class HomeController : BaseController
    {
        ...
        //TODO: obrisati posle testiranja, jer je pomereno da bude na strani Izvestaji o poslatim porukama
        //[AuthorizeUser(Roles = "Client,User")]
        //public ActionResult ScheduledSMS()
        //{
        //    int clientID = UserManager.FindById(User.Identity.GetUserId()).ClientID;

        //    var scheduledMessages = db.MessagesNumbers
        //        .Where(mn => mn.NumbersModel.ClientID == clientID &&
        //                mn.Message.Status == 2)
        //        .Select(mn => new
        //        {
        //            ID = mn.MessageID,
        //            NumberID = mn.NumberID,
        //            NumberTypeID = mn.NumberTypeID,
        //            Alphanumeric = mn.Message.Sender,
        //            Message = mn.Message.MessageText,
        //            SendDate = mn.Message.SendDate,
        //            User = mn.Message.User.UserName,
        //            MessageLength = mn.Message.MessageLength,
        //            Status = mn.Message.Status
        //        }).ToList();
        ...
        //    var numType = scheduledMessages
        //        .GroupBy(mn => new { mn.ID, mn.NumberTypeID })
        //        .DefaultIfEmpty()
        //        .Select(mn => new { ID = mn.Key.ID, NumberTypeID = mn.Key.NumberTypeID, Count = mn.Count() })
        //        .ToList();

        //    foreach (var m in scheduledMessages.GroupBy(mn => mn.ID).ToList())
        //    {
        //        var message = scheduledMessages.Where(mn => mn.ID == m.Key).FirstOrDefault();
        //        var vpnCount = numType.Find(nt => nt.ID == m.Key && (NumberType)nt.NumberTypeID == NumberType.VPN);
        //        var inMtsCount = numType.Find(nt => nt.ID == m.Key && (NumberType)nt.NumberTypeID == NumberType.U_MTS);
        //        var outMtsCount = numType.Find(nt => nt.ID == m.Key && (NumberType)nt.NumberTypeID == NumberType.VAN_MTS);
