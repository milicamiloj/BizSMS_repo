    ...
    {        
        ...
        public JsonResult CheckUsername(string Username, string initialUsername)
        {
            if (initialUsername == null)
            {
                ...
                //ukoliko se u donjem redu ostavi !u.IsDeleted onda validacija preskace user-e gde je Is_Deleted = 1 i baca gresku pri upisu dupliranog usera u bazu jer je kolona user u bazi setovana na unique
        ...
        public JsonResult NumberExist(string Number, string InitialNumber, int ClientID)
        {
            if (InitialNumber == null || Number != InitialNumber)
            {
                ...
                (number.ClientID == ClientID || (number.ClientID != ClientID && number.NumberTypeID == (int)NumberType.VPN)))
                    .FirstOrDefault();
        ...
        public JsonResult NumberExistInGroup(string Number, int GroupID)
        {
           int clientID = UserManager.FindById(User.Identity.GetUserId()).ClientID;
            ...
            //Sledeci upit ce pronaci takve iste aktivne brojeve u grupi i ako ih ima nece dozvoliti upis,samim tim podrazumeva se da ce dozvoliti upis deaktiviranog VPN broja (napomena: dozvolice i upis deaktiviranih brojeva koji nisu VPN tip, u bazi ih nema puno, trenutno 57, 03.03.2021)
            var numberInGroup = db.GroupNumbers.Where(gn => gn.GroupID == GroupID)
                .Join(db.Numbers.Where(n => n.Number == Number && n.ClientID == clientID && n.Active),
        ...
        [HttpPost]
        public JsonResult LimitNumberOfCharacters(string nameNumber)
        {
            //provera max broj karaktera koji moze da prodje, ako je vise od 30 vraca gresku 'EntityValidationErrors'
            if (nameNumber.Length > 30)
            {
                return Json(false);
            }
            else
